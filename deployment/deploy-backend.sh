...
if [[ "$ACCEPT_DEFAULTS" == false ]]; then
    ...
    prompt_with_default "Server Hostname" "${BACKEND_HOST:-backend.tailce422e.ts.net}" SERVER_HOST
    ...
    SERVER_HOST="${BACKEND_HOST:-backend.tailce422e.ts.net}"
    if [[ -z "${SSH_USER:-}" ]]; then
...
if [[ "$ACCEPT_DEFAULTS" == false ]]; then
    ...
    prompt_with_default "Kite Base URL" "${KITE_BASE_URL:-https://api.kite.trade}" KITE_BASE_URL
else
    if [[ -z "${KITE_API_KEY:-}" ]]; then
    ...
    KITE_BASE_URL="${KITE_BASE_URL:-https://api.kite.trade}"
...
if [[ "$ACCEPT_DEFAULTS" == false ]]; then
    ...
    prompt_with_default "CORS Allowed Methods" "${CORS_ALLOWED_METHODS:-GET,POST,PUT,DELETE,OPTIONS,PATCH}" CORS_ALLOWED_METHODS
    prompt_with_default "CORS Allow Credentials" "${CORS_ALLOW_CREDENTIALS:-true}" CORS_ALLOW_CREDENTIALS
else
...
log_info "-------------------"
if [[ "$ACCEPT_DEFAULTS" == false ]]; then
    prompt_with_default "Application Port" "${BACKEND_PORT:-8080}" APP_PORT
    prompt_with_default "Application User" "${BACKEND_USER:-moneytree-backend}" APP_USER
else
    APP_PORT="${BACKEND_PORT:-8080}"
    APP_USER="${BACKEND_USER:-moneytree-backend}"
fi
...
# Ensure we can write to the jar directory (copy will be done as current user, then moved)
run_sudo chmod 755 "${APP_DIR}/jar"
...
if run_sudo systemctl list-unit-files | grep -q "^${SERVICE_NAME}.service" && [[ -f "${APP_DIR}/app.jar" ]]; then
    IS_UPDATE=true
...
# Copy to temp location first, then move with sudo to ensure proper permissions
...
if command -v sshpass >/dev/null 2>&1; then
    ...
    sshpass -e ssh -o StrictHostKeyChecking=no "${SSH_USER}@${SERVER_HOST}" \
        "chmod +x /tmp/remote-deploy.sh && /tmp/remote-deploy.sh /tmp/moneytree-backend.jar '${ENV_CONTENT}' '${APP_USER}' '${APP_PORT}' '${SUDO_PASSWORD}'"
