...
prompt_with_default "Server Hostname" "socketengine.tailce422e.ts.net" SERVER_HOST
...
prompt_with_default "Kite WebSocket URL" "wss://ws.kite.trade" KITE_WEBSOCKET_URL

echo ""
log_info "WebSocket Configuration"
log_info "-----------------------"
prompt_with_default "WebSocket Allowed Origins" "https://moneytree.tailce422e.ts.net" WEBSOCKET_ALLOWED_ORIGINS
prompt_with_default "CORS Allowed Origins" "https://moneytree.tailce422e.ts.net" CORS_ALLOWED_ORIGINS
prompt_with_default "Max WebSocket Sessions" "1000" WEBSOCKET_MAX_SESSIONS
prompt_with_default "Max Message Size" "65536" WEBSOCKET_MAX_MESSAGE_SIZE

echo ""
log_info "Persistence Configuration"
log_info "-----------------------"
prompt_with_default "Batch Size" "1000" PERSISTENCE_BATCH_SIZE
prompt_with_default "Max Buffer Size" "100000" PERSISTENCE_MAX_BUFFER_SIZE
prompt_with_default "Batch Interval (minutes)" "15" PERSISTENCE_BATCH_INTERVAL_MINUTES

echo ""
log_info "Application Settings"
log_info "-------------------"
prompt_with_default "Application Port" "8081" APP_PORT
prompt_with_default "Application User" "moneytree-socketengine" APP_USER
...
if command -v sshpass >/dev/null 2>&1; then
    ...
    sshpass -e ssh -o StrictHostKeyChecking=no "${SSH_USER}@${SERVER_HOST}" \
        "chmod +x /tmp/remote-deploy.sh && sudo /tmp/remote-deploy.sh /tmp/socketengine.jar '${ENV_CONTENT}' '${APP_USER}' '${APP_PORT}'"
